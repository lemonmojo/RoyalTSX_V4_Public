...
- (void)paste:(id)sender {
    ...
    // iTermLib Edit: Might not come from a menu item
...
- (void)screenTerminalAttemptedPasteboardAccess {
    ...
    // iTermLib Edit: Different preferences path
    NSString *notice = @"The terminal attempted to access the clipboard but it was denied. Enable clipboard access in “Preferences - Connection Types - Terminal - Plugin Settings - Applications in terminal may access clipboard”.";
...
- (void)sessionViewDidChangeWindow {
    ...
    // iTermLib Edit:
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(sessionViewDidMoveToWindow:)]) {
        [self.delegate sessionViewDidMoveToWindow:self];
    }
}
