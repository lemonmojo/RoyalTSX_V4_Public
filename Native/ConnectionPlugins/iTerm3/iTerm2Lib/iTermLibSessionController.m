...
- (void)updateDisplay
{
    ...
    [self.session screenUpdateDisplay:YES];
}
...
- (PTYSession *)createTabWithProfile:(Profile *)profile
    ...
    // Passing syntehetic:YES causes the API server to not post a notification after creating the session which is desirable in the embedded case
    PTYSession *aSession = [[[PTYSession alloc] initSynthetic:YES] autorelease];
    ...
    if (aSession.screen) {
        ...
        [aSession startProgram:command environment:nil isUTF8:YES substitutions:substitutions];
    }
...
- (void)setupSession:(PTYSession*)aSession
    ...
    if ([aSession setScreenSize:sessionRect parent:self]) {
        ...
        // iTermLib TODO: Needs to be updated when upgrading to a new iTerm release
...
- (void)sessionViewDidMoveToWindow:(PTYSession *)session
{
    if (_observedWindow) {
        [NSNotificationCenter.defaultCenter removeObserver:self name:NSWindowDidBecomeKeyNotification object:_observedWindow];
        [NSNotificationCenter.defaultCenter removeObserver:self name:NSWindowDidResignKeyNotification object:_observedWindow];
...
- (void)setActiveSessionOnMainThread:(PTYSession*)session
{
    ...
    for (PTYSession* otherSession in self.delegate.allPTYSessions) {
        if ([otherSession.textview isFindingCursor]) {
            [otherSession.textview endFindCursor];
        }
        
        [otherSession.textview removeUnderline];
...
- (BOOL)sessionShouldAutoClose:(PTYSession *)session { return YES; }
...
// iTermLib: I don't think we need this as it apparently is only used to bury sessions
