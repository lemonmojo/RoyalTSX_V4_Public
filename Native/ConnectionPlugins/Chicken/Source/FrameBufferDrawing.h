...
- (void)drawRect:(NSRect)aRect at:(NSPoint)aPoint
{
    ...
    if ((aRect.size.width * aRect.size.height) > SCRATCHPAD_SIZE || YES) {
        ...
        if (pixels) {
            // TODO: This is a shitty workaround for the ERROR_CGDataProvider_BufferIsNotBigEnough error
            // Also see: https://forum.juce.com/t/crashes-with-juce-image-coregraphics-ios-12/29524
            BOOL TODO_MOJAVE_WORKAROUND = NO;
            
            if (@available(macOS 10.14, *)) {
                TODO_MOJAVE_WORKAROUND = YES;
            }
            
            if (TODO_MOJAVE_WORKAROUND) {
                NSData *data = [[NSData alloc] initWithBytesNoCopy:pixels length:sizeof(FBColor) * size.width * size.height freeWhenDone:NO];
                ...
                CGBitmapInfo bmpInfo = (CGBitmapInfo)kCGImageAlphaNoneSkipLast;
                
                CGImageRef image = CGImageCreate(size.width, size.height, 8, sizeof(FBColor) * 8, bpr, colorSpace, bmpInfo, dataProvider, nil, NO, kCGRenderingIntentDefault);
                
                if (!image) {
                    CGDataProviderRelease(dataProvider); dataProvider = nil;
                    CGColorSpaceRelease(colorSpace); colorSpace = nil;
                    [data release]; data = nil;
                ...
                NSRect rrr = NSMakeRect(0, 0, size.width, size.height);
                CGContextDrawImage(NSGraphicsContext.currentContext.CGContext, rrr, image);
        ...
        NSLog(@"TODO TODO TODO: Should never end up here");
...
// FX EDIT:
// The 'else' path in the original code caused a lot of crashes, i have no idea why but it seems to work with the method below
// I reverted back to the original code to have scaling support. If it continues to cause crashes, check this version below:
/* - (void)drawRect:(NSRect)aRect at:(NSPoint)aPoint
{
    NSRect r;
    int bpr;
