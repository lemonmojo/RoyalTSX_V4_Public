...
- (void)setRect:(NSData*)rectInfo
{
    ...
    if(theReader == nil) {
        ...
        if (lastEncoding >= sizeof(encodingNames) / sizeof(*encodingNames)) {
            NSString   *fmt = [[NSBundle bundleForClass:[FrameBufferUpdateReader class]] localizedStringForKey:@"UnknownRectangle" value:@"UnknownRectangle" table:nil];
            err = [NSString stringWithFormat:fmt, encoding];
        } else {
            NSString   *fmt = [[NSBundle bundleForClass:[FrameBufferUpdateReader class]] localizedStringForKey:@"UnknownRectangleLastEncoding" value:@"UnknownRectangleLastEncoding" table:nil];
            err = [NSString stringWithFormat:fmt, encoding,
