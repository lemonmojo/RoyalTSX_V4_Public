...
{
	int selectedRow = (int)[mProfileTable selectedRow];
	if ( selectedRow < 0 )
		return nil;
...
{
	NSArray *profileNames = [self _sortedProfileNames];
	NSInteger index = [profileNames indexOfObject: aProfile];
    if (index != NSNotFound)
        [self _selectProfileAtIndex: (int)index];
    else
...
- (NSArray*)_sortedProfileNames
{
	...
	NSInteger idx = [n indexOfObject: defaultProfileName];
