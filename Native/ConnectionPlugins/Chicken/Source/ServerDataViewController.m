...
{	
    ...
    if (mServer != nil)
	{
        ...
        if (port < DISPLAY_MAX) {
            ...
            str = [NSString stringWithFormat:ChickenVncFrameworkLocalizedString(@"PortNum", nil),
                    port];
        } else {
            [hostName setStringValue:[mServer host]];
            if (port >= PORT_BASE && port < PORT_BASE + DISPLAY_MAX) {
                NSString    *fmt = ChickenVncFrameworkLocalizedString(@"DisplayIsPort", nil);
...
- (void)loadProfileIntoView
{
    ...
    [profilePopup addItemWithTitle:ChickenVncFrameworkLocalizedString(@"EditProfiles", nil)];
...
- (void)descriptionFromPort
{
    ...
    if (port >= PORT_BASE && port < PORT_BASE + DISPLAY_MAX) {
        NSString    *fmt = ChickenVncFrameworkLocalizedString(@"PortIsDisplay", nil);

        str = [NSString stringWithFormat:fmt, port, port - PORT_BASE];
    } else {
        NSString    *fmt = ChickenVncFrameworkLocalizedString(@"PortNum", nil);
...
- (IBAction)connectToServer:(id)sender
{
	...
	[connectIndicatorText setStringValue:ChickenVncFrameworkLocalizedString(@"Connecting...", @"Connect in process notification string")];
	[connectIndicatorText display];

    [self disableControls];
    [connectBtn setTitle: ChickenVncFrameworkLocalizedString(@"Cancel", nil)];
...
- (void)connectionAttemptEnded
{
    ...
    [connectBtn setTitle: ChickenVncFrameworkLocalizedString(@"Connect", nil)];
