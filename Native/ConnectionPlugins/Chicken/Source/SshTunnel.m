...
- (id)initWithServer:(id<IServerData>)aServer delegate:(SshWaiter *)aDelegate
{
    if (self = [super init]) {
        ...
        [env setObject:[ChickenVncFrameworkBundle() pathForResource:@"ssh-helper"
                                                       ofType:@"sh"]
                forKey:@"SSH_ASKPASS"];
...
- (void)readFromSsh:(NSNotification *)notif
{
    ...
    if ([data length] == 0) {
        ...
        if ([notif object] == [sshErr fileHandleForReading] &&
             ...
             * recognized. */
            [self sshFailed:ChickenVncFrameworkLocalizedString(@"SshQuit", nil)];
        }
        return;
    }
...
- (void)processStringFromStderr:(NSString *)str
{
    ...
    // messages sent by ssh itself.
        ...
        NSString *fmt = ChickenVncFrameworkLocalizedString(@"NoNamedServer", nil);
        ...
        if ([str hasSuffix:@"Connection refused\r"]) {
            [self sshFailed:ChickenVncFrameworkLocalizedString(@"ConnectRefused", nil)];
        } else if ([str hasSuffix:@"Operation timed out\r"]) {
            [self sshFailed:ChickenVncFrameworkLocalizedString(@"ConnectTimedOut", nil)];
        } else {
        ...
        for (i = 0; i < sizeof(dict) / sizeof(*dict); i++) {
            if ([str hasPrefix:dict[i][0]]) {
                if (dict[i][1])
                    [self sshFailed:ChickenVncFrameworkLocalizedString(dict[i][1], nil)];
                return;
            }
        }
...
- (void)tunnelError:(NSString *)err
{
    ...
    for (i = 0; i < sizeof(dict) / sizeof(*dict); i++) {
        ...
        if (range.location != NSNotFound) {
            [delegate tunnelFailed:ChickenVncFrameworkLocalizedString(dict[i][1], nil)];
            return;
        }
    }

    [delegate tunnelFailed:ChickenVncFrameworkLocalizedString(@"TunnelOther", nil)];
}
